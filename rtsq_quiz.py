...
console = Console()
DB_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "_sqrt_results.db")
...
def parse_range_arg(arg):
    """Parse '1-6' style CLI arg into (1, 6)."""
    try:
        parts = arg.split("-")
        if len(parts) == 2:
...
def get_range():
    console.print(Panel("[bold cyan]√ Square Root Quiz[/bold cyan]", border_style="cyan"))
...
def truncate4(val):
    ...
    return __import__("math").floor(val * 10000) / 10000

def round4(val):
    return round(val, 4)

def check_answer(user_str, correct):
    """Accept if user typed value equals truncated correct answer (to 4dp, no rounding)."""
    try:
        user_val = float(user_str)
        return abs(user_val - truncate4(correct)) < 1e-9
    except ValueError:
        return False
...
def run_quiz(min_val=None, max_val=None, hints_on=None):
    ...
    for i, num in enumerate(numbers, 1):
        ...
        if is_correct:
            ...
            if hints_on:
                console.print(f"  [bold red]❌ Wrong![/bold red]   [yellow]Correct: [bold]{correct_display}[/bold][/yellow]")
            else:
                console.print("  [bold red]❌ Wrong![/bold red]")

        results.append((num, correct, raw if raw else '?', is_correct))
...
def show_results(results, hints_on, min_val, max_val):
    ...
    for i, (num, correct, user_ans, is_correct) in enumerate(results, 1):
        user_str    = f"[green]{user_ans}[/green]" if is_correct else f"[red]{user_ans}[/red]"
        correct_str = f"[green]{truncate4(correct):.4f}[/green]" if is_correct else f"[yellow]{truncate4(correct):.4f}[/yellow]"
        table.add_row(
            str(i), f"[bold]√{num}[/bold]",
            user_str, correct_str,
    ...
    if wrong_ones and not hints_on:
        ...
        for num, correct, user_ans, _ in wrong_ones:
            console.print(f"  [cyan]√{num}[/cyan] → you said [red]{user_ans}[/red], correct is [green]{truncate4(correct):.4f}[/green]")
